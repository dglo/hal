...
void hal_FPGA_DOMAPP_self_lc_mode(HAL_FPGA_DOMAPP_SELF_LC_MODES mode) {
  FPGA(LC_CONTROL) = ( FPGA(LC_CONTROL) & ~FPGABIT(LC_CONTROL, SELF_LC_MODE)) | mode;
}
...
int hal_FPGA_DOMAPP_self_lc_window(int self) {
   const int selfticks = ( (self - 25) / 25);
   
   if (selfticks<0 || selfticks>0x3f) return -1;
   
   FPGA(LC_CONTROL) = (FPGA(LC_CONTROL) & ~(FPGABIT(LC_CONTROL, SELF_LC_WINDOW)))| 
     (selfticks<<10);
