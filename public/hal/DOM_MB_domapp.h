...
/**
 * lookback memory pointer mask -- we only use the
 * lower 26 bits...
 */
#define HAL_FPGA_DOMAPP_LBM_MASK ((1<<26)-1)

/**
 * lookback memory pointer access mask -- we only use
 * the lower 22 bits (16M bytes) to access memory...
 */
#define HAL_FPGA_DOMAPP_LBM_ACCESS_MASK ((1<<22)-1)
...
/**
 * upward cable corrections
 */
typedef enum {
   HAL_FPGA_DOMAPP_LC_CABLE_SHORT = 0,
   HAL_FPGA_DOMAPP_LC_CABLE_LONG  = 1
} HAL_FPGA_DOMAPP_LC_CABLES;

/** 
 * set upward cable length correction
 *
 * \param cable one of HAL_FPGA_DOMAPP_LC_CABLES
 */
void
hal_FPGA_DOMAPP_lc_up_cable(HAL_FPGA_DOMAPP_LC_CABLES cable);

/** 
 * set cable length correction
 *
 * \param cable one of HAL_FPGA_DOMAPP_LC_CABLES
 */
void
hal_FPGA_DOMAPP_lc_down_cable(HAL_FPGA_DOMAPP_LC_CABLES cable);
...
/**
 * set calibration pulser rate -- pick the closest rate we can...
 *
 ...
 *
 * \param mask or of HAL_FPGA_DOMAPP_RATE_MONITORS
 */
void
 ...
 */
unsigned 
hal_FPGA_DOMAPP_spe_rate(void);

/**
 * get mpe rate
 */
unsigned
hal_FPGA_DOMAPP_mpe_rate(void);
...
/**
 * set supernova gate time
 *
 * \param time gate time in us 6554us .. 104858us in 6554us increments
 * \return 0 ok, non-zero, invalid gate time
 */
int
hal_FPGA_DOMAPP_sn_gate_time(int time);

/**
 * set supernova dead time
 *
